...
export default function ContributionForm({ user, onNavigate, onLogout }: ContributionFormProps) {
  ...
  const handleParticipationChange = (value: string) => {
    setFormData({ ...formData, participateAs: value })
  }

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    console.log("Contribution submission:", formData)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-gradient-to-r from-green-500 to-green-600 text-white">
        ...
        <div className="px-4 py-2 flex flex-wrap justify-center space-x-4 text-sm">
          ...
          <button onClick={() => onNavigate("contribution")} className="hover:underline font-bold">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {["Speaker", "Workshop Moderator"].map((option) => (
                      <label key={option} className="flex items-center space-x-2">
                        <input
                          ...
                          name="participateAs"
                          value={option}
                          checked={formData.participateAs === option}
            ...
            <div className="pt-4">
              <button
                type="submit"
                className="bg-green-600 hover:bg-green-700 text-white px-8 py-2 rounded font-medium transition-colors"
              >
                Submit
              </button>
            </div>
